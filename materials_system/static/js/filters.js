...
function get_url_without_filter(name) {
	var url = URI(window.location.href).removeSearch(name).addSearch(name,'');
...
function add_filter_params_to_url(url){
	var params =  URI(window.location.href).search();
	return URI(url).addSearch('params',params);
}
